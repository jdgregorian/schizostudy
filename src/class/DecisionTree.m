classdef (Abstract) DecisionTree
% Superclass for decision tree.
  ...
  properties (Abstract)
    ...
    trainvalues % values used for tree training
  ...
  methods
    function obj = DecisionTree(settings)
      obj.nodes = 1;
      obj.parent = 0;
      obj.children = [0, 0];
      ...
      % user defined properties
      obj.inForest = defopts(settings, 'inForest', false);
    end

    function obj = train(obj, data, values, settings)
      % training function
      ...
      [nSubjects, obj.dataDim] = size(data);
      assert(nSubjects == length(values), 'Number of datapoints and number of values are different')

      % as a part of forest
      if obj.inForest                      
        % remember only IDs of individuals
        obj.traindata = defopts(settings, 'usedInd', data);  
      else
        obj.traindata = data;
      end
      obj.trainvalues = values;
      ...
      if ischar(obj.maxSplit) && strcmp(obj.maxSplit, 'all')
        maxSplitNum = nSubjects;
      elseif isnumeric(obj.maxSplit)
        maxSplitNum = min(obj.maxSplit, nSubjects);
      else
